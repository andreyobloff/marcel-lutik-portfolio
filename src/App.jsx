...
const introSeriesDescriptions = {
  Derealisation: 'Derealisation — сюжетная серия о состоянии дереализации, где привычное пространство подъездов, отражений и бытовых маршрутов постепенно становится чужим и нестабильным.',
  Ether: 'Эфир — фотосессия в белом образе среди природных, архитектурных и пространственных антуражей, собранная вокруг холодного света и ощущения плотного воздуха.',
  'Эфир': 'Эфир — фотосессия в белом образе среди природных, архитектурных и пространственных антуражей, собранная вокруг холодного света и ощущения плотного воздуха.',
  Stilllife: 'Stilllife — работа с объектами, чёрными глянцевыми и матовыми поверхностями, золотом и символикой перехода от нигредо к вознесению.',
  Walk: 'Walk — визуальная прогулка, где маршрут, случайные детали, свет и городская среда складываются в последовательность наблюдений.',
};
...
function getIntroSeriesCopy() {
  const names = getSeriesNames();
  const descriptions = (manifest.series || []).map((series) => introSeriesDescriptions[series.title] || `${displayTitle(series.title)} — отдельная серия портфолио.`);
  return `${names ? `В портфолио собраны серии: ${names}.` : 'В портфолио собраны визуальные серии.'} ${descriptions.join(' ')}`;
}
function getHeroPhoto() { return manifest.series.find((series) => series.slug === 'derealisation')?.photos.find((photo) => photo.id === 'derealisation-15') || manifest.heroPhoto; }
...
function Header({ activeTheme, setThemeName }) {
  return <header className={`sticky top-0 z-40 border-b ${activeTheme.border} ${activeTheme.panel} backdrop-blur-xl`}><div className="flex items-center justify-between gap-4 px-4 py-4 md:px-8"><button type="button" onClick={navigateHome} className="flex items-center gap-3 text-left"><span className={`grid h-11 w-11 place-items-center border ${activeTheme.border} text-sm font-black tracking-[-0.08em]`}>ML</span><span><span className="block text-sm font-bold uppercase tracking-[0.22em]">{photographerName}</span><span className={`block text-xs uppercase tracking-[0.18em] ${activeTheme.muted}`}>Портфолио</span></span></button><div className="hidden items-center gap-2 md:flex"><button type="button" onClick={() => scrollToHomeSection('series')} className={`border border-transparent px-3 py-2 text-[10px] uppercase tracking-[0.18em] ${activeTheme.muted} hover:border-current/30 hover:text-current`}>Серии</button><button type="button" onClick={() => scrollToHomeSection('about')} className={`border border-transparent px-3 py-2 text-[10px] uppercase tracking-[0.18em] ${activeTheme.muted} hover:border-current/30 hover:text-current`}>О фотографе</button><button type="button" onClick={() => scrollToHomeSection('contacts')} className={`border border-transparent px-3 py-2 text-[10px] uppercase tracking-[0.18em] ${activeTheme.muted} hover:border-current/30 hover:text-current`}>Контакты</button><span className={`mx-1 h-6 border-l ${activeTheme.border}`} />{Object.entries(themes).map(([key, theme]) => <button key={key} type="button" onClick={() => setThemeName(key)} className={`border px-3 py-2 text-[10px] uppercase tracking-[0.18em] transition ${activeTheme.label === theme.label ? activeTheme.button : `border-transparent ${activeTheme.muted} hover:border-current/30`}`}>{theme.label}</button>)}</div></div></header>;
}

function Intro({ activeTheme }) {
  const heroPhoto = getHeroPhoto();
  const introText = getIntroSeriesCopy();
  return <section className="grid min-h-[72vh] border-b border-current/10 md:grid-cols-[1.12fr_0.88fr]"><div className="flex flex-col justify-between border-r-0 border-current/10 p-6 md:border-r md:p-10 lg:p-14"><div className="flex items-center gap-3 text-xs uppercase tracking-[0.28em] opacity-70"><Icon name="camera" /> Portfolio / Series</div><div className="py-16"><p className={`mb-6 text-xs uppercase tracking-[0.28em] ${activeTheme.accent}`}>Марсель Лютик</p><h1 className="max-w-5xl text-5xl font-semibold uppercase leading-[0.92] tracking-[0.018em] md:text-7xl lg:text-8xl">Портфолио</h1><p className={`mt-8 max-w-3xl text-lg leading-8 ${activeTheme.muted}`}>{introText}</p></div><div className="grid grid-cols-3 border border-current/15 text-center uppercase tracking-[0.18em]"><div className="border-r border-current/15 p-4"><div className="text-3xl tracking-[-0.05em]">{manifest.series.length}</div><div className={`mt-1 text-[10px] ${activeTheme.muted}`}>Серии</div></div><div className="border-r border-current/15 p-4"><div className="text-3xl tracking-[-0.05em]">{manifest.totalPhotos}</div><div className={`mt-1 text-[10px] ${activeTheme.muted}`}>Кадров</div></div><div className="p-4"><div className="text-3xl tracking-[-0.05em]">ML</div><div className={`mt-1 text-[10px] ${activeTheme.muted}`}>Работы</div></div></div></div><div className="group relative min-h-[460px] overflow-hidden">{heroPhoto ? <img src={assetUrl(heroPhoto)} alt={heroPhoto.title} className="h-full w-full object-cover grayscale contrast-125 transition duration-700 group-hover:scale-105 group-hover:grayscale-0 group-hover:contrast-100" /> : null}<div className="absolute inset-0 bg-gradient-to-t from-black/85 via-black/25 to-transparent" /><div className="absolute bottom-6 left-6 right-6 border border-white/20 bg-black/35 p-5 text-white backdrop-blur-sm"><p className="text-xs uppercase tracking-[0.24em] text-white/60">Портфолио</p><p className="mt-2 text-3xl uppercase tracking-[0.018em]">{photographerName}</p></div></div></section>;
}
...
function AboutSection({ activeTheme }) {
  ...
  return <section id="about" className="scroll-mt-28 border-t border-current/10 px-4 py-16 md:px-8 md:py-24"><div className="grid gap-10 md:grid-cols-[0.75fr_1.25fr] md:items-start"><div><p className={`mb-5 text-xs uppercase tracking-[0.28em] ${activeTheme.accent}`}>О фотографе</p><h2 className="text-4xl font-semibold uppercase leading-none tracking-[0.018em] md:text-6xl">Марсель Лютик</h2></div><div className="grid gap-8 lg:grid-cols-[0.82fr_1.18fr]"><div className={`group relative min-h-[460px] overflow-hidden border ${activeTheme.border} ${activeTheme.card}`}>{portrait ? <img src={assetUrl(portrait)} alt="Марсель Лютик" className="absolute inset-0 h-full w-full object-cover grayscale transition duration-700 group-hover:scale-105 group-hover:grayscale-0" /> : <div className={`flex h-full min-h-[460px] items-center justify-center p-8 text-center text-xs uppercase tracking-[0.18em] ${activeTheme.muted}`}>Фото фотографа появится после обновления локального архива</div>}<div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/10 to-transparent" /></div><div className={`border-y ${activeTheme.border} py-7 text-base leading-8 md:text-lg ${activeTheme.muted}`}><p>Марсель Лютик собирает портфолио как набор самостоятельных визуальных серий, в которых пространство, предметы, свет и поверхность становятся основными материалами кадра. Его работы держатся на внимании к состоянию места: подъезд, маршрут, натюрморт или природно-архитектурный антураж воспринимаются как среда, где можно увидеть напряжение, тишину, движение и внутренний сюжет.</p><p className="mt-6">В серии Derealisation бытовая реальность постепенно теряет устойчивость и превращается в лабиринт отражений, лестничных пролётов и тревожных знаков. Stilllife сосредоточена на объектах, фактурах чёрного, золоте и символике перехода; Walk работает с прогулкой как последовательностью наблюдений; Эфир обращается к портретному образу в холодном свете и пространстве, где воздух кажется плотным и почти вещественным.</p><p className="mt-6">Визуальный язык Марселя строится на строгой композиции, тёмной тональности, контрасте глянцевых и матовых поверхностей, а также на ощущении скрытого действия внутри неподвижного кадра. Это портфолио показывает не только отдельные фотографии, но и способ собирать серии как цельные визуальные высказывания.</p></div></div></div></section>;
}
